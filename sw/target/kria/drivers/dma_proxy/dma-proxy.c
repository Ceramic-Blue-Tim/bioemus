...
static int create_channel(struct platform_device *pdev, struct dma_proxy_channel *pchannel_p, char *name, u32 direction)
{
	 ...
	 */
	// master branch Github on 25 Oct 2022 : https://github.com/Xilinx-Wiki-Projects/software-prototypes
	// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	// Patch from https://www.spinics.net/lists/arm-kernel/msg779056.html
	// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	pchannel_p->channel_p = dma_request_slave_channel(&pdev->dev, name);
	if (pchannel_p->channel_p == NULL) {
	  printk(KERN_WARNING, "DMA channel request error with dma_request_slave_channel() retry with dma_request_chan() \n");
	  pchannel_p->channel_p = dma_request_chan(&pdev->dev, name);
	  if (pchannel_p->channel_p == NULL) {
	    dev_err(&pdev->dev, "DMA channel request error, no channel found\n");
	    rc = -ENODEV;
